...
//}
    ...
    array_shift($pages);
    array_push($pages,"viewAUsersTracks.php?user=$_GET[user]");
...
$entriesSql = "SELECT Songs.Thumbnail, Path, SongTitle, Description, Instruments, Genre, username, Songs.ID 
	       ...
	       AND users.id  = '".$_GET['user']."'" .(($getInstrument != "")? " 
	       AND users.id = '" . $_GET[user]  . "' 
          ...
          <div class="col-md-1"></div>
            <div class="col-md-10">
                <div class="panel panel-default">
                    ...
                    <?php
                        $findusernameSQL = "SELECT username FROM users WHERE id = $_GET[user]";
                        $usernames = $dbh->query($findusernameSQL);
                        foreach ($usernames as $username){
                            echo "<h2>Tracks By $username[username]</h2>";
                        }
                            ...
                            <nav>
                              <ul class="pagination">
                                <?php
                                    ...
                                    echo "<li><a href=\"?user=$_GET[user]&page=1&genre=$getGenre&instrument=$getInstrument\">&laquo;</a></li><li>";
                                if ($page != 1){
                                echo "<a href=\"?user=$_GET[user]&page=".($page-1)."&genre=$getGenre&instrument=$getInstrument\" aria-label=\"Previous\">";
                                } else {
                                echo "<a href=\"?user=$_GET[user]&page=$pages&genre=$getGenre&instrument=$getInstrument\" aria-label=\"Previous\">";
                                }
                                ...
                                for ($i=1; $i<$pages+1; $i++){
                                	echo "<li><a href=\"?user=$_GET[user]&page=$i&genre=$getGenre&instrument=$getInstrument\">$i</a></li>";
                                }
                                
                                if ($page != ($pages+1)){
                                echo "<li>
                               <a href=\"?user=$_GET[user]&page=".($page+1)."&genre=$getGenre&instrument=$getInstrument\" aria-label=\"Next\">";
                                } else {
                                echo "<li><a href=\"?user=$_GET[user]&page=1&genre=$getGenre&instrument=$getInstrument\" aria-label=\"Next\">";
                                }
